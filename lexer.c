...
// ex. LEX_GETC_IF(buffer, current_char, (50 > 90))
...
static struct token *token_make_string(char start_delim, char end_delim)
{
    ...
    for (; c != end_delim && c != EOF; c = nextc())
    {
        if (c == '\\')
        {
            // we nwwd to handel an escape character
            // "hello world\n";
            continue;
        }
...
const char *read_op()
{
    ...
    if (!single_operator)
    {
        if (!op_valid(ptr))
        {
            read_op_flush_back_but_keep_first(buffer);
            ptr[1] = 0x00;
        }
    }
...
struct token *token_make_multiline_comment()
{
    ...
    while (1)
    {
        ...
        if (c == EOF)
        {
            compiler_error(lex_process->compiler, "You did not closed multiline comment");
        }
        else if (c == '*')
        {
            nextc();
...
struct token *read_next_token()
{
    ...
    // We don't care about whitspace ignore them
    case ' ':
    case '\t':
